...
export const MindmapProvider = ({ children }) => {
  ...
  const updateNodeHyperlink = async (hyperlink) => {
    if (!mindmapInstance || !selectedNode) {
      console.error(
        "Mindmap instance is undefined or no node has been selected"
      );
      return;
    }
    ...
    if (updateNode(updatedNodeData)) {
      ...
      toast(`Hyperlink for node '${selectedNode.topic}' updated successfully`, {
        autoClose: 1500,
      });
    } else {
